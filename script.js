...
function redirectToCart(button) {
    const name = button.getAttribute('data-name');
    const price = button.getAttribute('data-price');
    const qty = 1; // Default quantity, or you can fetch from an input field

    // Build the URL
    const url = `caart.html?name=${encodeURIComponent(name)}&price=${price}&qty=${qty}`;

    // Redirect to caart.html
...
function addToCart(form, event) {
    ...
    // Get product details
    const name = form.parentElement.getAttribute('data-name');
    const price = parseFloat(form.parentElement.getAttribute('data-price'));
    const quantity = parseInt(form.qty.value);
    ...
    // Redirect to caart.html
    window.location.href = 'caart.html';
}
